...
pub(super) fn command() -> Command {
    let command = Command::new("vimeflow")
...
fn write_requested_help(args: &[String], output: &mut impl Write) -> std::io::Result<bool> {
    ...
    // Drives `set_bin_name` below, so this is the program name every
    // subcommand's `Usage:` line shows.
    let mut path = vec!["vimeflow".to_string()];
    ...
    Command::new("agent")
        ...
        .subcommand(
            Command::new("read")
                ...
                .override_usage("vimeflow agent read <TARGET> [OPTIONS]")
        ...
        .subcommand(
            Command::new("prompt")
                ...
                .override_usage("vimeflow agent prompt <TARGET> <TEXT> [OPTIONS]")
                .arg(required("target", "TARGET"))
        ...
        .subcommand(
            Command::new("rename")
                ...
                .override_usage("vimeflow agent rename <TARGET> <NAME>|--clear")
                .arg(required("target", "TARGET"))
        ...
        .subcommand(
            Command::new("wait")
                ...
                .override_usage("vimeflow agent wait <TARGET> [OPTIONS]")
                .arg(required("target", "TARGET"))
                .arg(
                    option("until", "STATUS")
                        .action(ArgAction::Append)
        ...
        .subcommand(
            Command::new("attach")
                ...
                .override_usage("vimeflow agent attach <TARGET> [OPTIONS]")
                .arg(required("target", "TARGET"))
                .arg(flag("takeover")),
        )
        .subcommand(
            Command::new("start")
                ...
                .override_usage(
                    "vimeflow agent start <NAME> --kind <KIND> --pane <ID> [OPTIONS] [-- [AGENT_ARG]...]",
                )
...
mod tests {
    ...
    fn every_spec_subcommand_renders_short_and_long_help() {
        ...
        for path in paths {
            for flag in ["-h", "--help"] {
                let mut args = vec!["vimeflow".to_string()];
                args.extend(path.iter().cloned());
                args.push(flag.to_string());
                let mut output = Vec::new();
                assert!(
                    super::write_requested_help(&args, &mut output).unwrap(),
                    "help was not handled for vimeflow {} {flag}",
                    path.join(" ")
                );
                let output = String::from_utf8(output).unwrap();
                assert!(
                    output.contains(&format!("Usage: vimeflow {}", path.join(" "))),
                    "unexpected help for vimeflow {}: {output}",
                    path.join(" ")
                );
            }
        }
    ...
    fn agent_rename_requires_exactly_one_name_or_clear() {
        ...
        assert!(String::from_utf8(help)
            .unwrap()
            .contains("Usage: vimeflow agent rename <TARGET> <NAME>|--clear"));
    }
