...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: vimeflow agent explain <target> [--json|--verbose]");
                eprintln!(
                    "usage: vimeflow agent explain --file PATH --agent LABEL [--json|--verbose]"
                );
                return Ok(0);
            }
            ...
            value => {
                if target.is_some() {
                    eprintln!("usage: vimeflow agent explain <target> [--json]");
                    return Ok(2);
                }
    ...
    let explain = if let Some(path) = file {
        if target.is_some() {
            eprintln!("usage: vimeflow agent explain --file PATH --agent LABEL [--json]");
            return Ok(2);
        }
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: vimeflow agent start <name> --kind KIND --pane ID [--timeout MS] [-- <agent-args...>]");
        return Ok(2);
    };
    let separator = args
        .iter()
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: vimeflow agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: vimeflow agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: vimeflow agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: vimeflow agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: vimeflow agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) = match super::parse_attach_target(
        args,
        "usage: vimeflow agent attach <target> [--takeover]",
    ) {
        Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: vimeflow agent wait <target> [--until STATUS]... [--timeout MS]");
        return Ok(2);
    };
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let [target, value] = args else {
        eprintln!("usage: vimeflow agent rename <target> <name>|--clear");
        return Ok(2);
    };
...
fn agent_prompt(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!(
            "usage: vimeflow agent prompt <target> <text> [--wait] [--until STATUS]... [--timeout MS]"
        );
        return Ok(2);
    };
...
fn agent_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: vimeflow agent send-keys <target> <key> [key ...]");
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: vimeflow agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
