...
- **フォンホームなし** — セルフアップデート、ホストされたマニフェストの取得、
  プロダクトのお知らせは意図的に無効化されています。このフォークが自分の上に
  純正の herdr をインストールすることは決してありません。

エージェントウォッチャー、自動タイトル、カード、キーボードナビゲーションは
**Unix 専用**（macOS と Linux）です。タブアイランドはすべてのプラットフォームで
ビルドされます。それ以外について、Windows は上流の機能セットをビルドします。
