...
| `src/app/state.rs` | Track title-sync generations, live Agents-card and compact-rail presentation state, and non-blocking watcher telemetry snapshots. | `e006a1ea`, `b8a6406c`, `95a7db2a`, compact-rail numbers, compact-rail agent marks (this commit) |
...
| `docs/next/website/src/data/config-reference.json` | Add the native agent watcher, title-sync, Agent-card, and compact-rail configuration keys to the generated user reference snapshot. | `18a6a734`, P5, compact-rail numbers, compact-rail agent marks (this commit) |
...
| `src/config.rs` | Export the fork's Agents-card view and compact-rail leading configuration types. | `b8a6406c`, compact-rail agent marks (this commit) |
