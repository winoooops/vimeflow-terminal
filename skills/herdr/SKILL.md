---
name: herdr
description: "Control Vimeflow, a terminal multiplexer for coding agents. Use only when the user explicitly mentions Vimeflow or asks to use Vimeflow to inspect or control panes, tabs, workspaces, commands, or another agent. Do not use merely because a task could benefit from a background terminal, delegation, or parallel work. Requires HERDR_ENV=1."
---
...
Vimeflow organizes terminals into workspaces, tabs, and panes, recognizes coding agents running inside panes, and exposes the current session through the `vimeflow` CLI.

The fork uses the `vimeflow` executable and retains `HERDR_*` variables for session routing.

Before issuing any control command, verify that this agent is running inside a Vimeflow-managed pane:

```bash
test "${HERDR_ENV:-}" = 1
```

If the check fails, say that you are not running inside Vimeflow and stop. Do not inspect or control the focused Vimeflow session from outside Vimeflow.

When the check passes, the `vimeflow` binary in `PATH` talks to the current session. Use it to inspect neighboring work, create terminal layout, start agents and commands, read output, and wait for state changes.
...
vimeflow worktree
vimeflow terminal
vimeflow notification
vimeflow integration
vimeflow session
```

Do not run bare `vimeflow` for discovery; it launches or attaches the TUI. Do not probe a mutating nested command by omitting arguments. Commands such as `vimeflow workspace create` are valid with defaults and will execute.
...
A pane exists whether or not it contains an agent. `agent start` requires an existing available shell pane and never creates, splits, or moves layout. Use pane commands for ordinary processes. Use agent commands when Vimeflow must validate agent identity or interpret `idle`, `working`, `blocked`, `done`, and `unknown` lifecycle states.
...
`idle` means the agent is ready for input and its tab has been seen in the focused Vimeflow UI. `done` is the same underlying idle state after unseen background work finishes. Focusing the tab or targeting the pane or agent with a focus command marks it seen. CLI reads do not mark it seen. `blocked` means Vimeflow recognized an approval or question UI. `unknown` means an agent is present but Vimeflow cannot classify it confidently; it does not prove completion.
...
Vimeflow injects the caller's context into each managed pane:
...
```bash
vimeflow workspace list
vimeflow tab list --workspace "$HERDR_WORKSPACE_ID"
vimeflow pane current --current
vimeflow pane list --workspace "$HERDR_WORKSPACE_ID"
vimeflow agent list
```
...
```bash
vimeflow pane layout --pane "$HERDR_PANE_ID"
```
...
```bash
vimeflow pane split --current --direction right --cwd "$PWD" --no-focus
```
...
```bash
vimeflow agent start reviewer --kind codex --pane <returned-pane-id>
```

Use the kind requested by the user. Run `vimeflow agent` to inspect the installed kind list and options. Pass native agent arguments only after `--`:

```bash
vimeflow agent start reviewer --kind codex --pane <returned-pane-id> -- <agent-args...>
```

`agent start` returns only after Vimeflow detects the expected agent in the same pane and considers it ready for interactive input. It defaults to a 30-second startup timeout.

Submit work through the agent surface:

```bash
vimeflow agent prompt reviewer "Review the current diff and report only actionable findings." --wait --timeout 120000
```
...
```bash
vimeflow agent send-keys reviewer esc
vimeflow agent send-keys reviewer ctrl+c
```

Vimeflow validates all keys before writing any bytes. Read the result through the resolved agent:

```bash
vimeflow agent get reviewer
vimeflow agent read reviewer --source recent-unwrapped --lines 120
```
...
```bash
vimeflow pane run <returned-pane-id> "just test"
vimeflow pane wait-output <returned-pane-id> --match "test result" --timeout 120000
vimeflow pane read <returned-pane-id> --source recent-unwrapped --lines 120
```
...
`--lines` asks Vimeflow for more rows from the pane's available screen and host scrollback. If increasing it does not reveal more of a completed response, the pane is probably running the agent on the terminal's alternate screen. Rows that leave the alternate screen do not enter Vimeflow's host scrollback, so a larger line count cannot recover them.
...
- Never run `vimeflow server stop` from an active session unless the user explicitly intends to stop the server and its pane processes.
- Never kill the main Vimeflow process. Use named test sessions for experiments that need an isolated server.
