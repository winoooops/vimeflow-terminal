...
Labels mode truncates the active tab label to 16 terminal cells. `ui.island.caps` defaults to `"round"`; Ghostty, Kitty, and WezTerm render the caps natively, while `"square"` restores the flat fallback. `ui.island.motion` defaults to `"smooth"`; use `"steps"` for whole-cell motion or `"off"` as the reduced-motion escape. All five settings reload live.
