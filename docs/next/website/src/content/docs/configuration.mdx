...
Labels mode truncates the active tab label to 16 terminal cells. `ui.island.caps` defaults to `"round"`; Ghostty, Kitty, and WezTerm render the caps natively, while `"square"` restores the flat fallback. All four settings reload live.
