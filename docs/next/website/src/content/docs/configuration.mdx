...
The desktop tab row uses a compact tab island by default. `ui.tab_bar_style` defaults to `"island"`; set it to `"classic"` to restore the upstream tab bar. `ui.island.position` defaults to `"center"` and also accepts `"left"`. `ui.island.display` defaults to `"dots"` and also accepts `"numbers"` or `"labels"`.

Labels mode shows every tab's name — the active tab as the accent pill and inactive tabs as text, each name truncated to a 14-cell text budget inside markers of up to 16 cells. In dots and numbers modes, `ui.island.active_title` defaults to `true` and shows the active tab title in the pill (replacing the number in numbers mode); set it to `false` for the untitled pill. `ui.island.caps` defaults to `"round"`; Ghostty, Kitty, and WezTerm render the caps natively, while `"square"` restores the flat fallback. `ui.island.motion` defaults to `"smooth"`; use `"steps"` for whole-cell motion or `"off"` as the reduced-motion escape.

Background agent completions and blocked states are retained as island notification records. `ui.island.arrivals` defaults to `"toast"`; set it to `"silent"` to collect records without showing arrival toasts. When records exist, the capsule shows `ui.island.bell` (default `"🔔"`) with the unread count; the emoji default needs no Nerd Font, and `bell = "!"` restores the ASCII form. Overrides must occupy one or two display cells.

Click the bell or press `prefix+i` (`keys.island_panel_toggle`) to open the notification panel. Use the arrow keys and Enter to jump to a record's workspace, tab, and pane; `r` marks all records read, `c` clears them, and Esc closes the panel.

Island notifications operate on the foreground client in multi-client sessions; a server-side notification feed is planned.

Install any Nerd Font and set it as your terminal font to use a custom icon, then set an override such as `bell = "󰂚"` under `[ui.island]`. All seven `[ui.island]` settings and `ui.tab_bar_style` reload live.

### Agent card traces

An expanded agent card lists its recent tool calls. Click one to select it, and click the selected row again to open a detail panel showing status, duration, timestamp, and the full retained argument preview — pretty-printed when it parses as JSON.

Press `prefix+a` (`keys.focus_agents`) to drive the sidebar from the keyboard. The mode captures keys and has two zones, shown in the mode bar:

| Zone | Keys |
| --- | --- |
| Cards | `j`/`k` move the cursor, `l` descends into traces, Enter focuses that pane and exits, `h`/Esc leaves |
| Traces | `j`/`k` move the selection, `o`/Enter opens the detail panel, `h` returns to cards, Esc leaves |
| Detail panel | `j`/`k` scroll, Esc closes the panel and keeps your place |

Moving the card cursor deliberately does **not** change pane focus — walking the list would otherwise switch workspace and tab on every keystroke. The card under the cursor expands while you move; Enter is the only key that commits focus.

While a trace is selected the card renders its whole retained ring instead of the usual five rows. Only settled calls that carry a tool-use id are selectable; the rest still render but are skipped by `j`/`k` and ignored by clicks.
...
Desktop in-app toasts default to `ui.toast.herdr.position = "island"`: centered on the capsule, below a top tab bar or above a bottom tab bar, with a plain top-center fallback in classic mode or when no capsule is laid out; `"bottom-right"` restores the corner placement, while mobile keeps its bottom banner and clipboard feedback retains its own position setting.
